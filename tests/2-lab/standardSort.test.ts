import { standardSort } from '../../2-lab/standardSort';
...
describe('testing index file', () => {
  test('standard test', () => {
    ...
    console.log(
      ...
      checkTimeExecutingFunc(() => standardSort(testArr))
    );
    expect(standardSort(testArr)).toStrictEqual(testArr.sort((a, b) => a - b));
  });
  ...
  test('empty array', () => {
    const testArr = [];
    expect(standardSort(testArr)).toStrictEqual([]);
  });
});
